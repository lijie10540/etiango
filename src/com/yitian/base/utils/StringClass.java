...
//        string = string.replaceAll("'", "&apos;");
	...
	public static String getReplaceString(String longstring, Map replaceStringMap) {
    	Object[][] objects=CollectionTool.getMapKeyValue(replaceStringMap);
...
//        }
    ...
    @SuppressWarnings("unchecked")
	public static List<String> getRealPaths(Class theClass){
    	List<String> realPathList=new ArrayList<String>();
    	Enumeration<URL> enumerations=null;
    	try{
        	enumerations=theClass.getClassLoader().getResources("/");
        	if(enumerations!=null){
        		while(enumerations.hasMoreElements()){
        			URL url=enumerations.nextElement();
        			String theUrl=StringClass.getFormatpathNoFileStart(url.toString());
        	    	if(SystemProperty.WINDOWS && theUrl.length()>0 && theUrl.startsWith("/")){
    	...
    	if(realPathList.size()<=0){
    		realPathList.add(getRealPath(theClass));
    	}
	...
	public static String getRealPath(Class clazz){
    	String realPath=getFormatPath(clazz.getResource("/").getPath());
    ...
    public static String getRealPath(){
    	String realPath=getFormatPath(StringClass.class.getResource("/").getPath());
    	if(SystemProperty.WINDOWS && realPath.length()>0 && realPath.startsWith("/")){
    		realPath=realPath.substring(1);
    	}
    	try {
...
//		System.out.println(getFormatPath("file://////d:/abc/efg//dsfdsf\\1.xml"));
		...
		String normalString="就算离开房间打扫发";
		try{
			String utf8String=URLEncoder.encode(normalString,"utf-8");
			System.out.println(utf8String);
			System.out.println(URLDecoder.decode(utf8String, "utf-8"));
		}catch(Exception e){
