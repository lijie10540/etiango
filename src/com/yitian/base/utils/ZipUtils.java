...
public class ZipUtils {
	...
	static public byte[] unZipEntry(byte[] zipData, String entryName) {
		try {
			...
			if (entryName != null) {
				while ((zipEntry != null)
						&& !(zipEntry.getName().equals(entryName))) {
			...
			while (bytesReadThisTime != -1) {
				bytesReadThisTime = zipInputStream.read(classByteBuffer, 0,
						classByteBuffer.length);
	...
	static public String zipString(String s) {
		return zipString(s, false);
	}

	static public String zipString(String s, boolean linebreak) {
		byte[] zip = null;
		try {
		...
		return StringUtils.encodeBase64(zip, linebreak);
	}

	static public String unZipString(String s) {
		try {
			byte[] bs = StringUtils.decodeBase64(s);
	...
	/*
	 ...
	 try {
	 FileInputStream f = new FileInputStream("c:/winzip.log");
	 byte [] bs= new byte [f.available()];
	 f.read(bs);
	 f.close();
	 byte [] zip=zipData(bs);
	 byte [] unzip = unZipData(zip);
	 System.out.println("size="+bs.length + "  afterzip="+zip.length +" after unzip="+ unzip.length);
	 String s= new String(unzip,"GBK");
	 s= zipString(s);
	 System.out.println(unZipString(s));
